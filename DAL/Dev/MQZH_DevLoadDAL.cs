...
namespace MQDFJ_MB.DAL.Dev
{
    ...
    public partial class MQZH_DevDAL : ObservableObject
    {
        ...
        /// 载入E01动态风压参数
        /// </summary>
        private void LoadDTFYParam()
        {
            string settingsUsingNO = "Using";

            //查找是否存在指定配置，若不存在则从Default拷贝新建
            try
            {
                bool isExistsInE01 = PublicData.DevDB.Queryable<MQZH_DevModel_DTFYSMParam>().AS("E01MQZH_DevModel_DTFYSMParam").Any(it => it.Group == settingsUsingNO);

                if (!isExistsInE01)
                {
                    bool isExistsDefultInE01 = PublicData.DevDB.Queryable<MQZH_DevModel_DTFYSMParam>().AS("E01MQZH_DevModel_DTFYSMParam").Any(it => it.Group == "Default");
                    if (!isExistsDefultInE01)
                    {
                        MessageBox.Show("E01默认参数丢失无法重建", "错误提示");
                        //  LogHelper.WriteErrLog("装置载入，E01表中未找到在用参数，且默认数据丢失，无法重建");//输出日志
                        return;
                    }

                    MQZH_DevModel_DTFYSMParam settingsDefault = PublicData.DevDB.Queryable<MQZH_DevModel_DTFYSMParam>().AS("E01MQZH_DevModel_DTFYSMParam").First(it => it.Group == "Default"); //查询第一条数据
                    settingsDefault.Group = settingsUsingNO;
                    PublicData.DevDB.Insertable<MQZH_DevModel_DTFYSMParam>(settingsDefault).AS("E01MQZH_DevModel_DTFYSMParam").ExecuteCommand();
                    MessageBox.Show("E01表中未找到在用配置参数，已重新建立！", "错误提示");
                    //  LogHelper.WriteErrLog("装置载入，E01表中未找到在用配置参数，已重新建立");//输出日志
                }
            }
            catch (Exception e)
            {
                MessageBox.Show(e.ToString(), "错误提示");
                // LogHelper.WriteErrLog("载入Dev时检查E01表格错误", e);//输出日志
            }

            //载入E01表中的参数
            try
            {
                MQZH_DevModel_DTFYSMParam devLoaded = PublicData.DevDB.Queryable<MQZH_DevModel_DTFYSMParam>().AS("E01MQZH_DevModel_DTFYSMParam").First(it => it.Group == settingsUsingNO);   //查询第一条数据
                PublicData.Dev.DtfysmParam = devLoaded;
            }
            catch (Exception e)
            {
                MessageBox.Show(e.ToString(), "错误提示");
                // LogHelper.WriteErrLog("载入E01时错误", e);//输出日志
            }
        }
