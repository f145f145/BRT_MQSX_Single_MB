...
namespace MQDFJ_MB.DAL.Dev
{
    ...
    public partial class MQZH_DevDAL : ObservableObject
    {
        ...
        /// 装置数据库初始化、读取数据
        /// </summary>
        private void DataBaseInit_Dev()
        {
            try
            {
                //创建sql打印输出，调试用
               PublicData.DevDB.Aop.OnLogExecuting = (sql, pars) =>
                {
                ...
                //检查D01表是否存在或未空
                bool isExistsD01 = PublicData.DevDB.DbMaintenance.IsAnyTable("E01MQZH_DevModel_DTFYSMParam", false);
                if (!isExistsD01)
                {
                    PublicData.DevDB.CodeFirst.As<MQZH_DevModel_DTFYSMParam>("E01MQZH_DevModel_DTFYSMParam").InitTables<MQZH_DevModel_DTFYSMParam>();
                    MessageBox.Show("数据库中未找到D01表，已重新创建，D01表所有数据已丢失！", "错误警告");
                 //   LogHelper.WriteErrLog("DAL初始化，数据库中未找到D01表，已重新创建");//输出日志
                }

            }
            catch (Exception e)
            {
                //  LogHelper.WriteErrLog("Dev数据库初始化错误", e);//输出日志
                MessageBox.Show(e.ToString());
            }
        }
