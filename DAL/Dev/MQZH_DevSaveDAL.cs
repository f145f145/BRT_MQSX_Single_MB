...
namespace MQDFJ_MB.DAL.Dev
{
    ...
    public partial class MQZH_DevDAL : ObservableObject
    {
        ...
        private void SavePressCtlSettings()
        {
            ...
            try
            {
                ...
                if (checkExistC14Row == null)
                {
                    ...
                    MQDFJ_MB.MQZH_DB_DevDataSet.C14压力控制参数Row newDevC14Row = C14Table.NewC14压力控制参数Row();
        ...
        /// 保存E01动态风压参数
        /// </summary>
        private void SaveDTFYParam()
        {
            try
            {
               PublicData.DevDB.Storageable<MQZH_DevModel_DTFYSMParam>(PublicData.Dev.DtfysmParam).ExecuteCommand(); //存在则更新，不存在则插入
            }
            catch (Exception e)
            {
                MessageBox.Show(e.ToString());
                //  LogHelper.WriteErrLog("保存D02表错误", e);//输出日志
            }
        }
        #endregion
