 ...
 * 创建时间：2021/12/10 20:17:36
 * 描述：
 * 数据转换静态类
 * ==================================================================================
 * 修改标记
 * 修改时间			修改人			版本号			描述
 * 2021/12/10 20:17:36		郝正强			V1.0.0.0
 *
 ************************************************************************************/
...
namespace MQZHWL.BLL
{
    ...
    /// </summary>
    public static class MQZH_ValueCvtBLL
    {
        ...
        public static ushort Bools2Ushort(bool[] array)
        {
            if (array != null && array.Length > 0)
            {
                ushort u16 = 0;
                for (int i = 0; i < Math.Min(array.Length,16); i++)
                {
                    if (array[i])
                    {
                        ushort nn = (ushort)(1 << i);//左移一位，相当于×2
                        u16 += nn;
                    }
                }
                return u16;
